    ...
    def get_hwid():
        if platform.system() == "Linux":
            ...
            try:
                ...
                if len(hwid) < 20:
                    raise ValueError("SID too short (likely running under Wine)")
            except Exception:
                # Fallback for environments where win32security/WMI doesn't
                # return a proper SID (e.g. running under Wine on Linux).
                # Produces a stable 64-char hex ID based on machine + MAC + username.
                import uuid, hashlib
                raw = f"{platform.node()}-{uuid.getnode()}-{winuser}"
